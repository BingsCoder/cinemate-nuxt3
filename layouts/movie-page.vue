<template>
    <div>
        ...
        <div v-else class="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-5 gap-4">
            <MovieCard v-for="movie in movies" :title="movie.title" :secdonaryTitle="movie.release_date"
...
const page = useState(`moviepage-${apiPath}`, () => 1)
const movies = useState(`movie-${apiPath}`, () => [])

const maxPage = ref(1)

const { data, error } = await useFetch('/api/movie/list', {
    query: {
        path: apiPath,
        page: page,
